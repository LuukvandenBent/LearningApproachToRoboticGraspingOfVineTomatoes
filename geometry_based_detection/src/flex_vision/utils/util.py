...
def plot_timer(timer_dict, N=1, threshold=0, ignore_key=None, pwd=None, name='time', title='time', startangle=-45):
    ...
    values = np.array(list(timer_dict.values()))
    ...
    labels = np.array(list(timer_dict.keys()))
    ...
    l =  sorted(zip(values_keep, labels_keep))
    values_keep, labels_keep = zip(*l)
    
    values_keep = list(values_keep)
    labels_keep = list(labels_keep)
